...
export function modifyWeather(player: Player) {
    ...
    const sliderItem = Array.of(tr("plugins.modifyWeather.sliderItem.0"), tr("plugins.modifyWeather.sliderItem.1"), tr("plugins.modifyWeather.sliderItem.2"));
    ...
    player.sendForm(fm, (player2, data: Array<null | number>) => {
        ...
        mc.setWeather(data[1] as 0 | 1 | 2) ? player2.tell(gmTell + tr("plugins.modifyWeather.success", sliderItem[data[1]])) : player2.tell(gmTell + tr("plugins.modifyWeather.fail", sliderItem[data[1]]));
    });
}
