...
export class dataOperation {
    static init() {
        try {
            if (!fileOperation.hasConfig()) fileOperation.setConfig(JSON.stringify(__inits.config));
            if (!fileOperation.hasData(pluginFile.bindcmd)) fileOperation.setData(pluginFile.bindcmd, JSON.stringify([]));
            if (!fileOperation.hasData(pluginFile.blackcmd)) fileOperation.setData(pluginFile.blackcmd, JSON.stringify([]));
            if (!fileOperation.hasData(pluginFile.block)) fileOperation.setData(pluginFile.block, JSON.stringify(__inits.block));
            if (!fileOperation.hasData(pluginFile.potion)) fileOperation.setData(pluginFile.potion, JSON.stringify(__inits.potion));
            if (!fileOperation.hasData(pluginFile.rule)) fileOperation.setData(pluginFile.rule, JSON.stringify(__inits.rule));
            if (!fileOperation.hasData(pluginFile.ui)) fileOperation.setData(pluginFile.ui, JSON.stringify(__inits.ui));
            return true;
        } catch (err) {
            parseError(err);
            return false;
    ...
    static load() {
        try {
            this.init();
            config = JSON.parse(fileOperation.getConfig());
            return true;
        } catch (err) {
            parseError(err);
            return false
