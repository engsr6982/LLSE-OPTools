...
/// <reference path="c:\LLSE_Aids/dts/helperlib/src/index.d.ts"/> 
...
function isForms(pl) {
    ...
    pl.sendForm(fm, (pl, id) => {
        if (id == null) return Close_Tell(pl);
        const Online_Players = Func_Module.GET_ALL_ONLINE_PLAYERS(); let opl;
        switch (id) {
            case 0:
                Forms_0(pl);
                function Forms_0(pl) {
        ...
        switch (res) {
            case true:
                ...
                pl.sendForm(fm, (pl, dt) => {
                    ...
                    mc.getPlayer(Online_Players[dt[0]]).addEffect(Number(PotionData[ID].type), Number(dt[2]) * 20, Number(dt[3]), Boolean(dt[4]).valueOf());
...
function BindCmd() {
    ...
    const CmdJurisdiction = (Type) => {
        switch (Type) {
            ...
            case "Admin": return 'PermType.GameMasters';
            default: return 'PermType.GameMasters';
        }
    };
